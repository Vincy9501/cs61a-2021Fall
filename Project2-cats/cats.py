...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    return [paragraphs[k] for paragraphs[k] in paragraphs if select(k)]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        return 0.0
    percentage = sum([1 for i in range(len(reference_words)) if i < len(reference_words) and typed_words[i] == reference_words[i]])/ len(typed_words)
...
def feline_flips(start, goal, limit):
    ...
    def count(start1, goal1, num, total):
        length = min(len(start1) - len(goal1))
        if num == length and total <= limit:
            return total
        if num == length and total > limit:
            return limit + 1
        if num < length:
            if start1[num] != goal1[num]:
                total += 1
        return count(start1, goal1, num+1, total)
    return count(start, goal, 0, 0)+abs(len(start)-len(goal))
    # END PROBLEM 6

    # Maybe the amount of computation isn't minimum.
    # The two following solutions use list to realize recursion.
    ...
    # Limit value changes in each recursion call and
        it will return limit + 1 while counts are larger than limit. 
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        fastest[times.index(min(times))].append(word_at(match, i))
    return fastest
    # END PROBLEM 10
